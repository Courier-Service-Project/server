...
module.exports = {
  ...
  getDistance: (req, res) => {
    const sender = req.body.Slocation;
    const Reciever = req.body.Rlocation;

    //console.log(sender, Reciever);

    getDistance(sender, Reciever, (error, results) => {
      if (error) {
        res.json({
          success: 0,
          message: error,
          message2: "Error getDistance",
        });
      }
      if (results.length > 0) {
